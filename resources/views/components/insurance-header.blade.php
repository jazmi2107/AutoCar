<!-- Insurance Company Header Component -->
<style>
...
<header>
            ...
            <div class="user-menu-container">
                <button class="profile-trigger" id="userMenuBtn">
                    @php
                        ...
                        $currentCompany = $currentUser->insuranceCompany ?? null;
                        $hasProfilePicture = $currentCompany && $currentCompany->profile_picture && \Storage::disk('public')->exists($currentCompany->profile_picture);
                        $profilePicUrl = $hasProfilePicture 
                            ? asset('storage/' . $currentCompany->profile_picture) 
                            : 'https://ui-avatars.com/api/?name=' . urlencode($currentCompany->company_name ?? $currentUser->name) . '&background=random';
                    @endphp
                    <img src="{{ $profilePicUrl }}" alt="Profile" onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($currentUser->name) }}&background=random'">
                </button>
                <div class="dropdown-menu" id="userDropdown">
                    <a href="{{ route('insurance_company.profile') }}">My Profile</a>
    ...
    <nav>
        <ul>
            <li><a href="{{ route('insurance_company.dashboard') }}" class="{{ request()->routeIs('insurance_company.dashboard') ? 'active' : '' }}">DASHBOARD</a></li>
            <li><a href="{{ route('insurance_company.requests') }}" class="{{ request()->routeIs('insurance_company.requests*') ? 'active' : '' }}">REQUESTS</a></li>
        </ul>
    </nav>
</header>
