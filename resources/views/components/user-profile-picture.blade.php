@php
    ...
    if($user && $user->driver && $user->driver->profile_picture) {
        ...
        if(file_exists($picturePath)) {
            ...
            $profilePicturePath = $user->driver->profile_picture;
    ...
    elseif($user && $user->mechanic && $user->mechanic->profile_picture) {
        ...
        if(file_exists($picturePath)) {
            $hasProfilePicture = true;
            $profilePicturePath = $user->mechanic->profile_picture;
...
@if($hasProfilePicture)
    <img src="{{ asset('storage/' . $profilePicturePath) . $cacheBuster }}" alt="Profile" onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&size={{ $size }}&background=random'" {{ $attributes }}>
@else
    <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&size={{ $size }}&background=random" alt="Profile" {{ $attributes }}>
@endif
