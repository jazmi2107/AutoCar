...
<head>
    ...
    <title>User Management - AutoCar Admin</title>
    ...
    <style>
        ...
        .stat-mini i { font-size: 2rem; color: #f8c300; margin-bottom: 10px; }
        ...
        .user-avatar { width: 45px; height: 45px; border-radius: 50%; border: 2px solid #f8c300; }
        .user-details h4 { margin: 0; color: #fff; font-size: 1rem; }
        .user-details p { margin: 3px 0 0; color: #888; font-size: 0.85rem; }
        
        .role-badge { display: inline-block; padding: 5px 12px; border-radius: 3px; font-size: 0.75rem; font-weight: bold; text-transform: uppercase; }
        .role-user { background: #2196F3; color: #fff; }
        .role-driver { background: #2196F3; color: #fff; }
        .role-mechanic { background: #9c27b0; color: #fff; }
        .role-insurance { background: #ff9800; color: #000; }
        .role-admin { background: #f44336; color: #fff; }
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <h1><i class="fas fa-users" style="color: #f8c300;"></i> User Management</h1>
        ...
        @if($users->isEmpty())
            <div class="empty-state">
                ...
                <p>{{ request('search') ? 'Try adjusting your search' : 'Start by creating your first user' }}</p>
            </div>
        @else
            <div class="stats-overview">
                ...
                <div class="stat-mini">
                    ...
                    <h3>{{ $users->where('created_at', '>=', now()->subDays(7))->count() }}</h3>
                    <p>New This Week</p>
                </div>
                <div class="stat-mini">
                    <i class="fas fa-chart-line"></i>
                    <h3>{{ $users->sum('assistance_requests_count') }}</h3>
                    <p>Total Requests</p>
                </div>
            </div>
                ...
                <tbody>
                    @foreach($users as $user)
                        <tr>
                            <td>
                                <div class="user-info">
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&background=random" alt="Avatar" class="user-avatar">
                            ...
                            <td>
                                <div class="action-buttons">
                                    ...
                                    <form id="delete-form-{{ $user->id }}" action="{{ route('admin.users.delete', $user->id) }}" method="POST" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-delete" onclick="showDeleteModal({{ $user->id }}, '{{ $user->name }}', '{{ $user->email }}')">
            ...
            @if($users->hasPages())
                <div class="pagination">
                    ...
                    @else
                        <a href="{{ $users->appends(request()->query())->previousPageUrl() }}"><i class="fas fa-chevron-left"></i> Prev</a>
                    @endif
                    ...
                    @if($users->hasMorePages())
                        <a href="{{ $users->appends(request()->query())->nextPageUrl() }}">Next <i class="fas fa-chevron-right"></i></a>
                    @else
    ...
    <script>
        ...
        function showDeleteModal(userId, userName, userEmail) {
            ...
            document.getElementById('deleteUserName').innerHTML = userName + '<br><span style="color: #888; font-size: 0.9rem;">' + userEmail + '</span>';
