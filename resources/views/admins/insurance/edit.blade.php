...
<head>
    ...
    <title>Edit Insurance Company - AutoCar Admin</title>
    ...
    <style>
        ...
        .info-box { background: #1a3a52; padding: 15px 20px; border-radius: 5px; margin-bottom: 25px; border-left: 4px solid #f8c300; }
        .info-box i { margin-right: 10px; color: #f8c300; }
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <div>
                <div class="breadcrumb">
                    <a href="{{ route('admin.insurance') }}"><i class="fas fa-shield-alt"></i> Insurance Companies</a> / Edit Company
                </div>
        ...
        <div class="info-box">
            ...
            <strong>Note:</strong> Leave password fields empty if you don't want to change the password. Fill both password fields only if you want to update the password.
        </div>

        <div class="form-card">
            <form action="{{ route('admin.insurance.update', $company->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="registration_number" name="registration_number" value="{{ old('registration_number', $company->registration_number) }}" required>
                    </div>

                    <div class="form-group">
                        <label for="email"><i class="fas fa-envelope"></i> Email Address *</label>
                        <input type="email" id="email" name="email" value="{{ old('email', $company->user->email) }}" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="approval_status" name="approval_status" required>
                            <option value="pending" {{ old('approval_status', $company->approval_status) == 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="approved" {{ old('approval_status', $company->approval_status) == 'approved' ? 'selected' : '' }}>Approved</option>
                            <option value="rejected" {{ old('approval_status', $company->approval_status) == 'rejected' ? 'selected' : '' }}>Rejected</option>
                        </select>
                    </div>
