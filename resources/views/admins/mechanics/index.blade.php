...
<head>
    ...
    <title>Mechanic Management - AutoCar Admin</title>
    ...
    <style>
        ...
        .stat-mini i { font-size: 2rem; color: #9c27b0; margin-bottom: 10px; }
        ...
        .mechanic-info { display: flex; align-items: center; gap: 15px; }
        .mechanic-avatar { width: 45px; height: 45px; border-radius: 50%; border: 2px solid #9c27b0; }
        .mechanic-details h4 { margin: 0; color: #fff; font-size: 1rem; }
        .mechanic-details p { margin: 3px 0 0; color: #888; font-size: 0.85rem; }
        ...
        .status-unavailable { background: #9e9e9e; color: #fff; }
...
<body>
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="modal-btn modal-btn-confirm" onclick="confirmDelete()">
                    <i class="fas fa-trash"></i> Delete Mechanic
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="container">
        <div class="page-header">
            <h1><i class="fas fa-user-cog" style="color: #9c27b0;"></i> Mechanic Management</h1>            <a href="{{ route('admin.mechanics.create') }}" class="btn btn-primary">
                <i class="fas fa-plus-circle"></i> Add New Mechanic
            </a>        </div>
        ...
        @if($mechanics->isEmpty())
            <div class="empty-state">
                ...
                <p>{{ request('search') || request('status') ? 'Try adjusting your filters' : 'No mechanics registered yet' }}</p>
            </div>
        @else
            <div class="stats-overview">
                <div class="stat-mini">
                    <i class="fas fa-user-cog"></i>
                    <h3>{{ $mechanics->total() }}</h3>
                    <p>Total Mechanics</p>
                </div>
                <div class="stat-mini">
                    <i class="fas fa-check-circle"></i>
                    <h3>{{ $mechanics->where('availability_status', 'available')->count() }}</h3>
                    <p>Available</p>
                </div>
                <div class="stat-mini">
                    <i class="fas fa-clock"></i>
                    <h3>{{ $mechanics->where('availability_status', 'unavailable')->count() }}</h3>
                    <p>Unavailable</p>
                </div>
                <div class="stat-mini">
                    <i class="fas fa-wrench"></i>
                    <h3>{{ $mechanics->where('availability_status', 'busy')->count() }}</h3>
                    <p>Busy</p>
                </div>
            </div>
                        ...
                        <tr>
                            <td>
                                <div class="mechanic-info">
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode($mechanic->user->name) }}&background=9c27b0" alt="Avatar" class="mechanic-avatar">
                                    <div class="mechanic-details">
                                        <h4>{{ $mechanic->user->name }}</h4>
                                        <p>{{ $mechanic->user->email }}</p>
                                    </div>
                                </div>
                            </td>
                            <td>{{ $mechanic->insuranceCompany->company_name ?? 'N/A' }}</td>
                            <td><i class="fas fa-phone" style="color: #9c27b0; margin-right: 5px;"></i>{{ $mechanic->phone_number ?? 'N/A' }}</td>
                            <td><span class="status-badge status-{{ $mechanic->availability_status }}">{{ ucfirst($mechanic->availability_status) }}</span></td>
                            <td>{{ $mechanic->created_at->format('M d, Y') }}</td>
                            <td>
                                <div class="action-buttons">
                                    <a href="{{ route('admin.mechanics.show', $mechanic->id) }}" class="btn btn-view">
                                        <i class="fas fa-eye"></i> View
                                    </a>
                                    <a href="{{ route('admin.mechanics.edit', $mechanic->id) }}" class="btn btn-edit">
                                        <i class="fas fa-edit"></i> Edit
                                    </a>
                                    <form id="delete-form-{{ $mechanic->id }}" action="{{ route('admin.mechanics.delete', $mechanic->id) }}" method="POST" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-delete" onclick="showDeleteModal({{ $mechanic->id }}, '{{ addslashes($mechanic->user->name) }}', '{{ addslashes($mechanic->insurance_name ?? 'N/A') }}')">
            ...
            @if($mechanics->hasPages())
                <div class="pagination">
                    @if($mechanics->onFirstPage())
                        <span class="disabled"><i class="fas fa-chevron-left"></i> Prev</span>
                    @else
                        <a href="{{ $mechanics->appends(request()->query())->previousPageUrl() }}"><i class="fas fa-chevron-left"></i> Prev</a>
                    @endif
                    
                    @foreach($mechanics->getUrlRange(1, $mechanics->lastPage()) as $page => $url)
                        @if($page == $mechanics->currentPage())
                            <span class="active">{{ $page }}</span>
                        @else
                            <a href="{{ $mechanics->appends(request()->query())->url($page) }}">{{ $page }}</a>
                        @endif
                    @endforeach
                    
                    @if($mechanics->hasMorePages())
                        <a href="{{ $mechanics->appends(request()->query())->nextPageUrl() }}">Next <i class="fas fa-chevron-right"></i></a>
                    @else
    ...
    <script>
        ...
        function showDeleteModal(mechanicId, mechanicName, companyName) {
            deleteFormId = 'delete-form-' + mechanicId;
            document.getElementById('deleteMechanicName').innerHTML = mechanicName + '<br><span style="color: #888; font-size: 0.9rem;">' + companyName + '</span>';
