...
<head>
    ...
    <title>Mechanic Approvals - Admin Dashboard</title>
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <div>
                <h1><i class="fas fa-user-check" style="color: #f8c300;"></i> Mechanic Approvals</h1>
                <p>Review and approve new mechanic registrations</p>
            </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="action-buttons">
                                    <button type="button" class="btn btn-success" onclick="openApproveModal({{ $mechanic->id }}, '{{ addslashes($mechanic->user->name) }}')">
                                        <i class="fas fa-check"></i> Approve
                                    </button>
                                    <button type="button" class="btn btn-danger" onclick="openRejectModal({{ $mechanic->id }}, '{{ addslashes($mechanic->user->name) }}')">
    ...
    <div id="approveModal" class="modal-overlay">
        <div class="modal-content">
            <div class="modal-header">Approve Mechanic</div>
            <div class="modal-body">
                Are you sure you want to approve <strong id="approveName" style="color: #fff;"></strong>?
                <p style="margin-top: 10px; font-size: 0.9rem; color: #aaa;">This will grant them access to the mechanic dashboard.</p>
            </div>
    ...
    <div id="rejectModal" class="modal-overlay">
        <div class="modal-content" style="border-color: #f44336;">
            <div class="modal-header" style="color: #f44336;">Reject Mechanic</div>
            <div class="modal-body">
    ...
    <script>
        function openApproveModal(id, name) {
            document.getElementById('approveForm').action = "{{ url('admin/mechanics') }}/" + id + "/approve";
            document.getElementById('approveName').innerText = name;
            document.getElementById('approveModal').style.display = 'flex';
        }

        function openRejectModal(id, name) {
            document.getElementById('rejectForm').action = "{{ url('admin/mechanics') }}/" + id + "/reject";
